...
定义并导出所有游戏常量、关卡几何数据、物理参数。`level.ts` 中的 R(x,y,w,h) 工厂函数将原始坐标转换为 Rect 碰撞体数组，`initECSFromLevel()` 将静态几何（尖刺）和实体生成描述（移动平台/激光/光球/检查点/NOVA 星）实例化为 ECS 实体。`background.ts` 在模块加载时用种子 RNG 生成固定序列的视差形状。
