...
<details>
<summary>systems/ui — 菜单 / 准备 / 暂停 / 大厅 / HUD / 小地图 / 操作说明</summary>

本目录存放所有用户界面绘制与场景构建。每个场景模块独立一个文件，由 `scenes.ts` 组合根注册到 `core/uiComponent` 的 UIManager。

场景模块：
- **menu.ts** — 开始菜单（标题 + 霓虹流光按钮 + 背景极光/流星/漂浮几何动画 + 操作说明入口）
...
> 预制体图鉴已迁移至 `工具/gallery`（单文件 HTML `prefab-gallery.html` + `sync-prefabs.mjs` 同步脚本）。
</details>
...
├── icons.ts         # 道具图标（光球/双跳票/钩锁/护盾/加速）：HUD 背包栏共用
...
**共享层**：`theme.ts` 提供字体/字号/颜色 token；`primitives.ts` 提供玻璃面板/遮罩/标题排版/返回按钮/入场计时/hover 复位；`icons.ts` 提供道具图标（HUD 槽位共用）。各场景模块（menu / prepare / pause / lobby / hud / dev / instructions）优先复用共享层，消灭平行实现。

**menu.ts** — 构建开始菜单（标题/副标题/操作说明/目标提示/霓虹开始按钮 + 操作说明/设置入口）。**prepare.ts** — 构建准备界面（选图/选人卡片，卡片列表由 maps / CHARACTERS 动态驱动）。**hud.ts** — drawHUD：绘制左上角统计面板（光球/速度/跳跃/物理/加速/用时/死亡）、底部操作提示、Toast 浮动提示、胜利横幅与统计数据。drawMinimap：绘制 252px 宽的小地图（平台/尖刺/激光/光球/检查点/NOVA/玩家/视口框）。**pause.ts** / **lobby.ts** / **dev.ts** / **instructions.ts** — 通过 `core/uiComponent` 的 Button/TextInput/Toggle 组件构建各 UI 场景。**scenes.ts** 汇总注册全部场景到 UIManager，syncUI() 每帧根据 gs.screen + lobby + prepare 状态自动切换当前场景。
