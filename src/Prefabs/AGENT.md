...
Prefabs/
├── AGENT.md        # 本文件（目录说明）
├── Animations/     # 实体动画控制器注册表（已实现）：registry + getAnimOutput 输出辅助
...
├── Scenes/         # 场景道具建模（已实现）：绘制层 + 各场景实体的 ECS 工厂（*Entity.ts）+ itemsAnimators（动画控制器）
