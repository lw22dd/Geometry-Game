...
- **绘制层**：atmosphere（视差/曳光/粒子/文字提示）、hazards（尖刺 + 激光）、items（光球/检查点/NOVA 星/拾取物）、platforms（平台/边框/装饰/网格）、tracks（轨道）、theme（霓虹风格统一令牌 + 绘制原语）。
...
├── theme.ts            # 霓虹风格统一令牌：底色 / 描边 / 光晕档位 / 渐变色（绘制原语）
├── tracks.ts           # 绘制：轨道
