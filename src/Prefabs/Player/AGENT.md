...
├── playerEntity.ts    # 规划中（尚未创建）—— 玩家 ECS 实体接线，见架构决策记录
...
- **实体注册（规划中）**：玩家物理 ECS 接线时，用 `core/ecs` 的 SoA 组件（Position/Velocity/Collider/
  Player/PlayerControl/PlayerInput）承载玩家状态，`qLocalPlayer` 查询已就绪。
...
- 玩家实体接线（规划中）完成后：实体交由 `core/ecs` world 管理，`systems/level` 碰撞系统与 `systems/player` 查询读取。

# 架构决策记录（2025-06 · 玩家 ECS 接线前，已定案）

1. **玩家实体跨 clearWorld 存活**：切图重建只清场景实体，玩家实体跨图保留
   （`resetToSpawn` 清字段、不清实体），保证 `qLocalPlayer` 跨图可用。
