...
import type {
  InputKeys, NetOrbState, NetItemState, NetCipherState, NetChestState, NetPlayerState, RemotePlayerInfo,
} from '../types';
...
interface NetEvents {
  ...
  /** 收到房主权威状态（仅非房主） */
  state: (seq: number, players: NetPlayerState[], orbs: NetOrbState[], items: NetItemState[], ciphers: NetCipherState[], chests: NetChestState[], gt: number, gotN: number, deaths: number, win: boolean) => void;
  /** 收到房主事件 */
...
class NetClient {
  ...
  /** 房主发送权威状态广播 */
  sendHostState(
    seq: number, players: NetPlayerState[], orbs: NetOrbState[], items: NetItemState[], ciphers: NetCipherState[], chests: NetChestState[],
    gt: number, gotN: number, deaths: number, win: boolean,
  ): void {
    if (room.role !== 'host') return;
    this.sendJSON({
      type: 'host_state', seq, players, orbs, items, ciphers, chests, gt, gotN, deaths, win,
    });
  }
  ...
  private handleMessage(raw: string): void {
    ...
    switch (msg.type) {
      ...
      case 'state':
        emit('state', msg.seq, msg.players, msg.orbs, msg.items || [], msg.ciphers || [], msg.chests || [], msg.gt, msg.gotN, msg.deaths, msg.win);
        break;
