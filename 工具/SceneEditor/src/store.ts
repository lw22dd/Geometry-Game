...
export class EditorStore {
  ...
  setMode(mode: EditorMode): void {
    ...
    // 模式切换时清空选中与放置工具
    this.selection = [];
    this.tool = null; // 几何模式仅选择
    if (mode === 'objects') this.tool = this.objTool;
    if (mode === 'geometry') this.geomTool = 'select';
    this._notify();
  }
