/**
 * SceneEditor 入口 —— 引导编辑器、挂载事件、主循环。
 *
 * 定位：只摆放场景（objects 层）；结构底盘由 MVMap 导入（只读显示）。
 * 已移除几何「矩形画笔」绘制；几何层仅保留选择/移动/缩放/旋转。
 * 菜单栏、副工具栏、搜索、大纲树、小地图、Toast、复制/粘贴均保留。
 */
import './style.css';
...
import {
  ...
  renderGhost, renderMapBounds, updateStatusBar, renderHover,
  updateMouseStatus, renderFloor,
} from './render';
...
const drag: DragState = {
  kind: 'none', button: 0, sx: 0, sy: 0, wx: 0, wy: 0,
...
function handleObjMouseDown(wx: number, wy: number): void {
  if (store.objTool) {
    ...
    if (entry) {
      ...
      // 记录来源工具 id（区分同类多预设，如垂直/水平弹簧）
      (inst as { toolId?: string }).toolId = entry.toolId;
